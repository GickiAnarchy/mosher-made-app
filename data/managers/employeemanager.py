...
class EmployeeManager:
    ...
    def _get_row_index(self, name):
        ...
        names = self.ds.get_column_values("DATA", 1)
        try:
    ...
    def add(self, name, wage="$0.00"):
        ...
        current_names = self.ds.get_column_values("DATA", 1)
        ...
        self.ds.update_range("DATA", f"A{next_row}:B{next_row}", [[name, wage]])
    ...
    def update(self, name, new_name=None, new_wage=None):
        ...
        current_wage = new_wage if new_wage is not None else self.ds.get_cell_value("DATA", row, 2)
        
        self.ds.update_range("DATA", f"A{row}:B{row}", [[current_name, current_wage]])
    ...
    def delete(self, name):
        ...
        if row:
            ...
            self.ds.update_range("DATA", f"A{row}:B{row}", [["", ""]])
    ...
    def get(self, name):
        row = self._get_row_index(name)
        if row:
            wage = self.ds.get_cell_value("DATA", row, 2)
